...
class Day9():
    ...
    def visited_tiles_amount(self) -> int:
        ...
        for command in self.input:
            rope = self.do_command(rope, command)
        return len(self.visited_tiles)

    def do_command(self, rope: list[list[int]], command: list) -> list[list[int]]:
        direction = command[0]
        steps = int(command[1])
        for _ in range(steps):
            match direction.lower():
                ...
                case "d":
                    rope[0][0] += 1
            rope = self.move_tails(rope)
            if rope[-1] not in self.visited_tiles:
                self.visited_tiles.append(rope[-1])
        return rope

    def move_tails(self, rope: list[list[int]]) -> list[list[int]]:
        for i in range(len(rope) - 1):
            rope[i + 1] = self.move_tail(rope[i + 1], rope[i])
        return rope
